...
func (c *Client) f(id interface{}, n int) (f interface{}, err error) {
	...
	switch n {
	case 0:
		//fmt.Println("charpc---0")
		_, ok = f.(func([]interface{}))
	case 1:
		//fmt.Println("charpc---1")
		_, ok = f.(func([]interface{}) interface{})
	case 2:
		//fmt.Println("charpc---2")
		_, ok = f.(func([]interface{}) []interface{})
	default:
		panic("bug")
	}
...
// 屏蔽函数名输出
func ServerRouteInfo(ci interface{}) {
	hideMsgIds := []string{"internal.handleGetClubDeskList60",
		"internal.handleHeartBeat11",
	} //这里填写要屏蔽输出的信息

	unp := reflect.ValueOf(ci.(*CallInfo).f).Pointer()
	funcp := runtime.FuncForPC(unp)
	file, line := funcp.FileLine(unp)
	name := funcp.Name()
	//fmt.Println(name)
	//fmt.Printf("%s:%d: \n", file, line)
	...
	fmt.Println(textColor(strings.Repeat("*", 20)), textColor("TraceBegin"), textColor(strings.Repeat("*", 20)))
	fmt.Printf("%s %s: %s\n", textColor("*"), textColor("方法名称", textYellow), textColor(name, textPurple))
	if args := ci.(*CallInfo).args; len(args) > 0 {
		fmt.Printf("%s %s:%T\n%s %s:%v\n", textColor("*"), textColor("pType(参数类型)", textYellow), args[0], textColor("*"), textColor("pValue(参数值)", textYellow), args[0])
	}
	if cb := ci.(*CallInfo).cb; cb != nil {
		fmt.Printf("%s callback:%v\n", textColor("*"), cb)
	}
	fmt.Printf("%s %s: %s:%d\n", textColor("*"), textColor("所在位置", textYellow), file, line)
	fmt.Println(textColor(strings.Repeat("*", 20), textRed), textColor("TraceEnd", textCyan), textColor(strings.Repeat("*", 20), textRed))
}
...
func textColor(str string, color ...int) string {
	c1 := textGreen
	if len(color) > 0 {
		c1 = color[0]
	}
	return fmt.Sprintf("\x1b[0;%dm%s\x1b[0m", c1, str)
}
